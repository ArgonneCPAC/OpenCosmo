...
def read_indices(ds: Dataset, indices: np.array, range_: Optional[tuple[int, int]] = None) -> Column:
    if len(indices) == 0:
...
def write_indices(input_ds: Dataset, output_group: Group, indices: np.ndarray, range_: Optional[tuple[int,int]] = None):
    if len(indices) == 0:
    ...
    output_group.create_dataset(output_name, dtype=input_ds.dtype, data=data, compression=compression)
