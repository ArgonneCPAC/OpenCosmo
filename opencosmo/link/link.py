...
try:
    ...
    # If mpi4py is not available, set MPI to None
...
class LinkedCollection(dict):
    ...
    def open(cls, file: File, names: Optional[Iterable[str]] = None):
        ...
        for name, ds in datasets.items():
            try:
                ...
                output_datasets[name] = ds
        return cls(header, properties, output_datasets, links, len(properties))
    ...
    def write(self, file: File):
        ...
        if MPI is not None and MPI.COMM_WORLD.Get_size() > 1:
            rank_total_sizes = MPI.COMM_WORLD.allgather(self.__properties_total_length)
            offsets = np.insert(np.cumsum(rank_total_sizes), 0, 0)[:-1]
            idxs = idxs + offsets[MPI.COMM_WORLD.Get_rank()]
            # All ranks need all indices
            # Designed for small number of indices
            idxs = MPI.COMM_WORLD.allgather(idxs)
            idxs = np.concatenate(idxs)  # Gather all indices across ranks
            idxs = np.sort(idxs)
        keys = list(self.__aliases.keys())
        keys.sort()  # Sort the keys to ensure consistent ordering for MPI
        for key in keys:
            dataset = self.__datasets.get(key)
            if dataset is self.__properties:
                continue
            alias = self.__aliases[key]
            if isinstance(self.__linked[alias], dict):
                ...
                indices = self.__linked[alias][idxs]
...
def write_links_mpi(
    ...
    if MPI is None:
        # should never happen
        raise RuntimeError("MPI is not available, cannot write links with MPI")

    print(indices)
    assert False
    group = file.require_group("data_linked")
    rank = MPI.COMM_WORLD.Get_rank()
    lengths = MPI.COMM_WORLD.allgather(len(indices))
    ...
    for key in keys:
        ...
        if key == "sod_profile":
            ...
            group.create_dataset(f"{key}_start", shape=(total_length,), dtype=np.int64)
            group.create_dataset(f"{key}_size", shape=(total_length,), dtype=np.int64)
        MPI.COMM_WORLD.Barrier()  # Ensure all ranks are synchronized before writing

        if rank == 0:
            recv_data = np.empty(np.sum(lengths), dtype=indices.dtype)
            offsets = np.insert(np.cumsum(lengths), 0, 0)[:-1]
            recv_buffer = (recv_data, lengths, offsets, MPI.LONG)
        else:
