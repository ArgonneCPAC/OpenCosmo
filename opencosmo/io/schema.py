...
def validate_shapes(columns: list["ColumnSchema"], links: Iterable["LinkSchema"] = []):
    """
...
class FileSchema: 
    ...
    def allocate(self, group: h5py.File):
        if not isinstance(group, h5py.File):
            raise ValueError("File Schema allocation must be done at the top level of a h5py file!")
...
class DatasetSchema:
    def __init__(self, columns: Iterable["ColumnSchema"], links: Iterable["LinkSchema"] = [], indices: Iterable["IndexSchema"] = [], has_header: bool = False):
