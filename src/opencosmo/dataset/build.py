...
GroupedColumnData = dict[str, dict[str, np.ndarray]]


def build_dataset_from_data(
    data: GroupedColumnData | h5py.Group,
    ...
    if isinstance(data, dict):
        data = make_in_memory_h5_file_from_data(data)
...
def make_in_memory_h5_file_from_data(data: GroupedColumnData) -> h5py.File:
    name = uuid.uuid1()
    ...
    for group_name, columns in data.items():
        ...
        for column_name, column_data in columns.items():
            group.create_dataset(column_name, column_data)
    file.mode = "r"
    return file
